...
def ask(model_id, message, cookies=None, headers=None):
    ...
    for i, current_impersonate in enumerate(impersonates_to_try):
        try:
            logger.info(f"Trying impersonate {i+1}/{len(impersonates_to_try)}: {current_impersonate}")

            max_session_retries = 3
            last_resp = None
            for attempt in range(max_session_retries):
                session_id, user_msg_id, model_a_msg_id, data = build_payload()
                logger.info(f"Making HTTP request to lmarena.ai... (session attempt {attempt+1}/{max_session_retries})")
                ...
                try:
                    ...
                    if isinstance(err_payload, dict) and isinstance(err_payload.get('error'), str):
                        err_msg = err_payload.get('error')
                        if err_msg == 'Cannot select private models in non-battle mode':
                            logger.warning('Received non-battle private model error; aborting impersonate fallbacks')
                            raise Exception('Cannot select private models in non-battle mode')
                        if 'Evaluation session' in err_msg and 'already exists' in err_msg:
                            logger.warning(f"{err_msg} — regenerating UUIDs and retrying")
                            continue
            ...
            try:
                err_payload = json.loads(response.text)
                if isinstance(err_payload, dict) and err_payload.get('error') == 'Cannot select private models in non-battle mode':
                    logger.warning('Received non-battle private model error; aborting impersonate fallbacks')
                    raise Exception('Cannot select private models in non-battle mode')
            except json.JSONDecodeError:
                pass
            ...
            if response.status_code == 200:
                ...
                thinking, response_text, metadata = _parse_streaming_response(response.text)

                logger.info(f"Response parsed - thinking: {len(thinking)} chars, response: {len(response_text)} chars")
                ...
                logger.warning(f"Request failed with status {response.status_code}, trying next impersonate")
                continue

        except Exception as e:
            if 'Cannot select private models in non-battle mode' in str(e):
                logger.warning("Non-battle private model error encountered; stopping fallbacks")
                raise
    ...
    for imp in order:
        try:
            ...
            for attempt in range(max_session_retries):
                ...
                logger.info(f"image gen status={resp.status_code} via {imp} (session attempt {attempt+1}/{max_session_retries})")
                ...
                try:
                    ...
                    if isinstance(err_payload, dict) and isinstance(err_payload.get('error'), str):
                        ...
                        if err_msg == 'Cannot select private models in non-battle mode':
                            ...
                            raise Exception('Cannot select private models in non-battle mode')
                        if 'Evaluation session' in err_msg and 'already exists' in err_msg:
                            logger.warning('session already exists, regenerating UUIDs and retrying')
                            continue
                except json.JSONDecodeError:
                    pass
